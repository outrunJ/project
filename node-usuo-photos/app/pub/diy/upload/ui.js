...
function FileProgress(file, targetID) {
  ...
  if (!this.fileProgressWrapper.length) {
    ...
    var progressDesc = $("<td/>");
    var progressDescBox = $("<input/>");
    progressDescBox.addClass('progressDesc col-md-12');
    progressDesc.append(progressDescBox);
...
FileProgress.prototype.setComplete = function (up, info) {
  var td = this.fileProgressWrapper.find('td:eq(3)'),
  ...
  // **********
  var cargo = window.curCargo;
  var tr = this.fileProgressWrapper;
  var $desc = tr.find('td:eq(1) > input');
  cargo.photos.push({
    name: tr.find('td:eq(0)').html(),
    desc: $desc.val(),
    genre: $('#cargoGenre').val(),
    url: url,
  });

  $desc.replaceWith($desc.val());
  ...
  var showImg = $('<img src="/diy/upload/loading.gif"/>');

  progressNameTd.append(Wrapper);

  if (!isImg) {
    showImg.attr('src', '/diy/upload/default.png');
    ...
    $(img).on('load', function () {
      ...
      function initImg(url, key, height) {
        ...
        modalBody.find('img').attr('src', '/diy/upload/loading.gif');
